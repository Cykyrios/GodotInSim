...
func test_buffer_to_struct(
	...
	_test = assert_int(struct.gear_spare).is_equal(gear_spare)
	...
	_test = assert_vector(struct.gis_position).is_equal_approx(Vector2(struct.x, struct.y) / CarContact.POSITION_MULTIPLIER, epsilon * Vector2.ONE)
	_test = assert_float(struct.gis_speed).is_equal_approx(struct.speed / CarContact.SPEED_MULTIPLIER, epsilon)
	_test = assert_float(struct.gis_direction).is_equal_approx(struct.direction / CarContact.ANGLE_MULTIPLIER, epsilon)
	_test = assert_vector(struct.gis_acceleration).is_equal_approx(Vector2(struct.accel_right, struct.accel_forward) \
			/ CarContact.SPEED_MULTIPLIER, epsilon * Vector2.ONE)
	_test = assert_float(struct.gis_heading).is_equal_approx(struct.heading / CarContact.ANGLE_MULTIPLIER, epsilon)
	_test = assert_float(struct.gis_steer).is_equal_approx(struct.steer / CarContact.STEER_MULTIPLIER, epsilon)
