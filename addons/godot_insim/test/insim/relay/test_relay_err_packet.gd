...
var params_decode_packet := [
	[PackedByteArray([1, 255, 1, 3])],
	[PackedByteArray([1, 255, 200, 1])],
]
@warning_ignore("unused_parameter")
func test_decode_packet(buffer: PackedByteArray, test_parameters := params_decode_packet) -> void:
	var packet := InSimPacket.create_packet_from_buffer(buffer) as RelayERRPacket
	var _test: GdUnitAssert = assert_object(packet).is_instanceof(RelayERRPacket)
	...
	_test = assert_int(packet.error).is_equal(buffer[3])
	_test = assert_bool(packet.error in InSim.RelayError.values()).is_true()
