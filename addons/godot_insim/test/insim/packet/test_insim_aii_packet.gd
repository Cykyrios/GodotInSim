...
func test_decode_packet(buffer: PackedByteArray, test_parameters := [
	...
	_test = assert_int(packet.sp3).is_equal(buffer.decode_u8(OutSimMain.STRUCT_SIZE + 7))
	_test = assert_float(packet.rpm).is_equal(buffer.decode_float(OutSimMain.STRUCT_SIZE + 8))
	_test = assert_float(packet.spf0).is_equal(buffer.decode_float(OutSimMain.STRUCT_SIZE + 12))
	_test = assert_float(packet.spf1).is_equal(buffer.decode_float(OutSimMain.STRUCT_SIZE + 16))
	_test = assert_int(packet.show_lights).is_equal(buffer.decode_u32(OutSimMain.STRUCT_SIZE + 20))
	_test = assert_int(packet.spu1).is_equal(buffer.decode_u32(OutSimMain.STRUCT_SIZE + 24))
	_test = assert_int(packet.spu2).is_equal(buffer.decode_u32(OutSimMain.STRUCT_SIZE + 28))
	_test = assert_int(packet.spu3).is_equal(buffer.decode_u32(OutSimMain.STRUCT_SIZE + 32))
