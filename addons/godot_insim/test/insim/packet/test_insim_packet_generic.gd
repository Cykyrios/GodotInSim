...
func _test_receivable_sendable(packet: InSimPacket) -> void:
	...
	if packet.receivable:
		_test = (
			assert_bool(has_method("test_decode_packet"))
		...
		_test = (
			assert_bool(has_method("test_decode_packet"))
			.override_failure_message("Non-receivable packet should not have test_decode_packet()")
			.is_false()
		)
	if packet.sendable:
		_test = (
			assert_bool(has_method("test_encode_packet"))
		...
		if packet.get_property_list().any(func(property: Dictionary) -> bool:
			...
			_test = (
				assert_bool(has_method("test_encode_packet_gis"))
