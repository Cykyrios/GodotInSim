...
func test_decode_packet(buffer: PackedByteArray, test_parameters := buffers) -> void:
	...
	_test = assert_float(packet.gis_closing_speed).is_equal_approx(
		buffer.decode_u16(4) / InSimOBHPacket.CLOSING_SPEED_MULTIPLIER, epsilon
	)
	_test = assert_int(packet.time).is_equal(buffer.decode_u16(6))
	_test = assert_array(packet.object.get_buffer()).is_equal(
		buffer.slice(8, 8 + CarContObj.STRUCT_SIZE)
	)
