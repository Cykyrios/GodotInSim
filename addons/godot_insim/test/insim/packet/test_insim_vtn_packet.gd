...
func test_decode_packet(buffer: PackedByteArray, test_parameters := buffers) -> void:
	...
	if is_failure():
		return
	_test = assert_int(packet.size).is_equal(buffer.decode_u8(0) * InSimVTNPacket.SIZE_MULTIPLIER)
	...
	_test = assert_int(packet.spare2).is_equal(buffer.decode_u8(6))
	_test = assert_int(packet.spare3).is_equal(buffer.decode_u8(7))
	packet.fill_buffer()
