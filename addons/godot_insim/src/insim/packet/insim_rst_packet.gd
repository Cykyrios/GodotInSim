...
func _get_pretty_text() -> String:
	var session_type := (
		0 if race_laps == 0 and qual_mins == 0
		else 1 if race_laps == 0
		else 2
	)
	var session := (
		"Practice" if session_type == 0
		else "Qualifying" if session_type == 1
		else "Race"
	)
	var duration := (
		"" if session_type == 0
		else " (%d minutes)" % [qual_mins] if session_type == 1
		else " (%d %s)" % [
			race_laps if race_laps < 100
			else (100 + 10 * (race_laps - 100)) if race_laps <= 190
			else (race_laps - 190),
