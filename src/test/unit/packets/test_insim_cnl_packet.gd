...
func test_decode_packet(params: Array = use_parameters(params_decode_packet)) -> void:
	...
	assert_is(packet, InSimCNLPacket)
	if is_failing():
		return
	assert_eq(packet.size, buffer.decode_u8(0) * InSimCNLPacket.SIZE_MULTIPLIER)
	...
	assert_eq(packet.reason, buffer.decode_u8(4))
	assert_eq(packet.total, buffer.decode_u8(5))
