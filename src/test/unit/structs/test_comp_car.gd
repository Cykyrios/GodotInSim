...
var args: Array[Array] = [
	[378, 1, 34, 1, 192, -1734651, 16728913, 10448, 11118, 62087, 62581, 1660],
]
...
func test_buffer_to_struct(params: Array = use_parameters(params_buffer_to_struct)) -> void:
	...
	assert_eq(struct.x, params[1][5] as int)
	assert_eq(struct.y, params[1][6] as int)
	assert_eq(struct.z, params[1][7] as int)
	assert_eq(struct.speed, params[1][8] as int)
	assert_eq(struct.direction, params[1][9] as int)
	assert_eq(struct.heading, params[1][10] as int)
	assert_eq(struct.ang_vel, params[1][11] as int)
...
func test_struct_to_buffer(params: Array = use_parameters(params_struct_to_buffer)) -> void:
	...
	struct.x = params[0][5] as int
	struct.y = params[0][6] as int
	struct.z = params[0][7] as int
	struct.speed = params[0][8] as int
	struct.direction = params[0][9] as int
	struct.heading = params[0][10] as int
	struct.ang_vel = params[0][11] as int
...
func test_gis_struct_to_buffer(params: Array = use_parameters(params_struct_to_buffer)) -> void:
	...
	struct.position = params[0][3] as int
	struct.info = params[0][4] as int
	struct.gis_position = Vector3(params[0][5] as int, params[0][6] as int, params[0][7] as int) \
			/ CompCar.POSITION_MULTIPLIER
	struct.gis_speed = params[0][8] as float / CompCar.SPEED_MULTIPLIER
	struct.gis_direction = deg_to_rad(params[0][9] as float / CompCar.ANGLE_MULTIPLIER)
	struct.gis_heading = deg_to_rad(params[0][10] as float / CompCar.ANGLE_MULTIPLIER)
	struct.gis_angular_velocity = deg_to_rad(params[0][11] as float / CompCar.ANGVEL_MULTIPLIER)
