...
func test_buffer_to_struct(params: Array = use_parameters(params_buffer_to_struct)) -> void:
	...
	assert_eq(struct.sp2, params[1][2] as int)
	assert_eq(struct.steer, params[1][3] as int)
	assert_eq(struct.throttle_brake, params[1][4] as int)
	assert_eq(struct.clutch_handbrake, params[1][5] as int)
	assert_eq(struct.gear_spare, params[1][6] as int)
	assert_eq(struct.speed, params[1][7] as int)
	assert_eq(struct.direction, params[1][8] as int)
	assert_eq(struct.heading, params[1][9] as int)
	assert_eq(struct.accel_forward, params[1][10] as int)
	assert_eq(struct.accel_right, params[1][11] as int)
	assert_eq(struct.x, params[1][12] as int)
	assert_eq(struct.y, params[1][13] as int)
...
func test_struct_to_buffer(params: Array = use_parameters(params_struct_to_buffer)) -> void:
	...
	struct.sp2 = params[0][2] as int
	struct.steer = params[0][3] as int
	struct.throttle_brake = params[0][4] as int
	struct.clutch_handbrake = params[0][5] as int
	struct.gear_spare = params[0][6] as int
	struct.speed = params[0][7] as int
	struct.direction = params[0][8] as int
	struct.heading = params[0][9] as int
	struct.accel_forward = params[0][10] as int
	struct.accel_right = params[0][11] as int
	struct.x = params[0][12] as int
	struct.y = params[0][13] as int
...
func test_gis_struct_to_buffer(params: Array = use_parameters(params_struct_to_buffer)) -> void:
	...
	struct.info = params[0][1] as int
	struct.sp2 = params[0][2] as int
	struct.gis_steer = params[0][3] as float / CarContact.STEER_MULTIPLIER
	struct.throttle_brake = params[0][4] as int
	struct.clutch_handbrake = params[0][5] as int
	struct.gear_spare = params[0][6] as int
	struct.gis_speed = params[0][7] as float / CarContact.SPEED_MULTIPLIER
	struct.gis_direction = params[0][8] as float / CarContact.ANGLE_MULTIPLIER
	struct.gis_heading = params[0][9] as float / CarContact.ANGLE_MULTIPLIER
	# gis_acceleration = Vector2(acceleration_right, acceleration_forward)
	struct.gis_acceleration = Vector2(params[0][11] as int, params[0][10] as int) \
			/ CarContact.ACCELERATION_MULTIPLIER
	struct.gis_position = Vector2(params[0][12] as int, params[0][13] as int) \
