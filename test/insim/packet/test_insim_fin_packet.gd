...
func test_decode_packet(buffer: PackedByteArray, test_parameters := [
	...
	_test = assert_float(packet.gis_race_time) \
	...
	_test = assert_int(packet.sp_a).is_equal(buffer.decode_u8(12))
	...
	_test = assert_int(packet.sp_b).is_equal(buffer.decode_u8(15))
