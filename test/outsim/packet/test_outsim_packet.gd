...
func test_decode_packet(
	...
	if packet.outsim_options & OutSim.OutSimOpts.OSO_TIME:
		_test = assert_int(packet.outsim_pack.time).is_equal(buffer.decode_u32(offset))
		_test = assert_float(packet.outsim_pack.gis_time) \
				.is_equal_approx(buffer.decode_u32(offset) / OutSimPack.TIME_MULTIPLIER, epsilon)
		offset += 4
	if packet.outsim_options & OutSim.OutSimOpts.OSO_MAIN:
		_test = assert_array(packet.outsim_pack.os_main.get_buffer()) \
	...
	if packet.outsim_options & OutSim.OutSimOpts.OSO_INPUTS:
		_test = assert_float(packet.outsim_pack.os_inputs.throttle) \
				.is_equal_approx(buffer.decode_float(offset), epsilon)
		_test = assert_float(packet.outsim_pack.os_inputs.brake) \
				.is_equal_approx(buffer.decode_float(offset + 4), epsilon)
		_test = assert_float(packet.outsim_pack.os_inputs.input_steer) \
				.is_equal_approx(buffer.decode_float(offset + 8), epsilon)
		_test = assert_float(packet.outsim_pack.os_inputs.clutch) \
				.is_equal_approx(buffer.decode_float(offset + 12), epsilon)
		_test = assert_float(packet.outsim_pack.os_inputs.handbrake) \
	...
	if packet.outsim_options & OutSim.OutSimOpts.OSO_DRIVE:
		_test = assert_int(packet.outsim_pack.gear).is_equal(buffer.decode_u8(offset))
		_test = assert_int(packet.outsim_pack.sp1).is_equal(buffer.decode_u8(offset + 1))
		_test = assert_int(packet.outsim_pack.sp2).is_equal(buffer.decode_u8(offset + 2))
		_test = assert_int(packet.outsim_pack.sp3).is_equal(buffer.decode_u8(offset + 3))
		_test = assert_float(packet.outsim_pack.engine_ang_vel) \
				.is_equal_approx(buffer.decode_float(offset + 4), epsilon)
		_test = assert_float(packet.outsim_pack.max_torque_at_vel) \
				.is_equal_approx(buffer.decode_float(offset + 8), epsilon)
		offset += 12
	if packet.outsim_options & OutSim.OutSimOpts.OSO_DISTANCE:
		_test = assert_float(packet.outsim_pack.current_lap_distance) \
				.is_equal_approx(buffer.decode_float(offset), epsilon)
		_test = assert_float(packet.outsim_pack.indexed_distance) \
				.is_equal_approx(buffer.decode_float(offset + 4), epsilon)
		offset += 8
	if packet.outsim_options & OutSim.OutSimOpts.OSO_WHEELS:
		for i in 4:
			var wheel := packet.outsim_pack.os_wheels[i]
	...
	if packet.outsim_options & OutSim.OutSimOpts.OSO_EXTRA_1:
		_test = assert_float(packet.outsim_pack.steer_torque) \
				.is_equal_approx(buffer.decode_float(offset), epsilon)
		_test = assert_float(packet.outsim_pack.spare) \
				.is_equal_approx(buffer.decode_float(offset + 4), epsilon)
