...
Just like [OutGauge](../outgauge.md), you can create an :class_ref[OutSim] instance
entirely from code, or add it as a node in your scene and use <Code>@onready</Code>.
...
includes all data in the :class_ref[OutSimPacket]. Missing data will therefore be at their default values;
if you need to check which data is valid, the options value is included in the :class_ref[OutSimPacket].
...
We are going to use the :class_ref[CarInfo] class to load a `CAR_info.bin` file generated by pressing
<kbd>O</kbd> in the garage view, to retrieve the drive type. We will then use that to check only
the driven wheels in the :class_ref[OutSimPacket].
...
OutSim packets always include 4 :class_ref[OutSimWheel] objects, the order is given in the
[OutSimPacket.WheelIndex](/class_ref/OutSimPacket.mdx#enum_WheelIndex) enum: rear left, rear right,
...
:class_ref[OutSimMain] data of the :class_ref[OutSimPacket]:
...
RearLeft, and RearRight. Make all 4 of them unique and add <Code>@onready</Code> variables for them.
...
Now we just need to call this function instead of our earlier <Code>print("Spinning!")</Code>.
...
Some of the data available to OutSim is also available via [InSimMCIPackets](/class_ref/InSimMCIPacket.mdx),
albeit with less precision, but this allows you to work around the limitations of OutSim.
